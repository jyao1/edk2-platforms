  ...
  )
{
  ...
  ASSERT (Hob != NULL);
  PhysMemAddressWidth = ((EFI_HOB_CPU *) Hob)->SizeOfMemorySpace;
  ...
  if (PhysMemAddressWidth <= 39) {
    Pml4Entries = 1;
    PdpEntries = 1 << (PhysMemAddressWidth - 30);
    ASSERT (PdpEntries <= 0x200);
  } else {
    Pml4Entries = 1 << (PhysMemAddressWidth - 39);
  ...
  )
{
  ...
  //
  {
    ...
    DEBUG ((EFI_D_INFO, "%a: PhysMemAddressWidth=%d PeiMemoryCap=%u KB\n",
      __FUNCTION__, PhysMemAddressWidth, PeiMemoryCap >> 10));
